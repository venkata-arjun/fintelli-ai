...
Currency: ${currency}
Total Income (this month): ${currency} ${income.toFixed(2)}
Total Expenses (this month): ${currency} ${expenses.toFixed(2)}
...
export const generateBudgetAlert = async ({
  ...
  const percentUsed = budget > 0 ? ((spent / budget) * 100).toFixed(1) : "0.0";
...
Monthly Income (last 30 days): ${currency} ${income.toFixed(2)}
...

{
  ...
  "highlight": "Single short phrase capturing the key takeaway (e.g., 'Heavy on dining', 'Stable income', 'Dining dominates spending')"
}`;
...
export const analyzeBudgetList = async ({ budgets, currency }) => {
  const lines = (budgets || [])
    .map((b) => {
      ...
      return `Budget ID ${b.id} | Category: ${b.category_name} | Limit: ${currency} ${total.toFixed(
        2,
      )} | Spent: ${currency} ${spent.toFixed(2)} (${pct}% used)`;
    })
    .join("\n");
...
- status: "good" (well-paced, under target), "caution" (approaching limit or above 70%), or "concerning" (over budget or trending badly)
